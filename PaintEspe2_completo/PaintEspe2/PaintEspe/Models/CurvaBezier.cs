...
namespace PaintEspe.Models
{
    public class CurvaBezier : Figura
    {
        ...
        {
            int minX = Math.Min(Math.Min(P0.X, P1.X), Math.Min(P2.X, P3.X));
            int maxX = Math.Max(Math.Max(P0.X, P1.X), Math.Max(P2.X, P3.X));
            int minY = Math.Min(Math.Min(P0.Y, P1.Y), Math.Min(P2.Y, P3.Y));
            int maxY = Math.Max(Math.Max(P0.Y, P1.Y), Math.Max(P2.Y, P3.Y));
        ...
        {
            double u = 1 - t;
            double tt = t * t, uu = u * u, uuu = uu * u, ttt = tt * t;
        ...
        public bool ContienePuntoDeControl(Point punto, int tolerancia, out int indice)
        {
            ...
            for (int i = 0; i < puntos.Length; i++)
            {
                ...
                { indice = i; return true; }
            }
            indice = -1; return false;
        }
        ...
        {
            P0 = RotarPunto(P0, anguloDeg, centro);
            P1 = RotarPunto(P1, anguloDeg, centro);
            P2 = RotarPunto(P2, anguloDeg, centro);
            P3 = RotarPunto(P3, anguloDeg, centro);
        }

        public override void Escalar(double factorX, double factorY, Point centro)
        {
            P0 = EscalarPunto(P0, factorX, factorY, centro);
            P1 = EscalarPunto(P1, factorX, factorY, centro);
            P2 = EscalarPunto(P2, factorX, factorY, centro);
            P3 = EscalarPunto(P3, factorX, factorY, centro);
        }
    }
}
